...
@PipelineName("RXPResultPipelines")
public class RXPResultPipelines implements Pipeline<RXPBeanList> {
	
	@Override
	public void process(RXPBeanList RXPBeanList) {
		
		String title = "ID	DATA SET ID	HORMON	ORGAN	ELAPSED TIME	EXPERIMENT NAME	Cy3_SAMPLE NAME	Cy5_SAMPLE NAME	GRC#	Structure Development Stage	Anatomical Entity";
		...
		for (RXPBean RXPBean : RXPBeanList.getRxpBeans()) {
			String result = RXPBean.getId() + "\t" + RXPBean.getData_set_id() + "\t" + RXPBean.getHormon() + "\t" + RXPBean.getOrgan() + 
					"\t" + RXPBean.getElapsed_time() + "\t" +  RXPBean.getExperiment_name() + "\t" + RXPBean.getCy3_sample_name() +
